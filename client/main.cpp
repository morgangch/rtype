...
int main() {
    ...
    world.AddComponent<Common::Position>(player, 100.0f, 200.0f, 0.0f);
    world.AddComponent<Common::Velocity>(player, 0.0f, 0.0f, 250.0f);
    
    world.AddComponent<Common::Position>(enemy, 300.0f, 150.0f, 3.14f/2);
    world.AddComponent<Common::Velocity>(enemy, -50.0f, 0.0f, 100.0f);

    // Ajouter des composants spécifiques au client
    world.AddComponent<Client::Renderable>(player, "player.png", 64.0f, 64.0f, Client::RenderLayer::Entities);
    world.AddComponent<Client::Renderable>(enemy, "enemy.png", 48.0f, 48.0f, Client::RenderLayer::Entities);
    world.AddComponent<Client::Renderable>(ui_element, "health_bar.png", 200.0f, 20.0f, Client::RenderLayer::UI);

    // Ajouter caméra
    world.AddComponent<Client::Camera>(world.CreateEntity(), 1.0f);

    // Ajouter son
    world.AddComponent<Client::AudioSource>(player, "player_move.wav", Client::AudioType::SFX);
    ...
    for (int frame = 0; frame < 5; ++frame) {
        ...
        auto* positions = world.GetAllComponents<Common::Position>();
        if (positions) {
            for (const auto& pair : *positions) {
                ECS::EntityID entity = pair.first;
                auto* pos = pair.second.get();
                auto* vel = world.GetComponent<Common::Velocity>(entity);
        ...
        auto* renderables = world.GetAllComponents<Client::Renderable>();
        if (renderables) {
            std::cout << "  Rendering:" << std::endl;
            for (const auto& pair : *renderables) {
                ECS::EntityID entity = pair.first;
                auto* renderable = pair.second.get();
                auto* pos = world.GetComponent<Common::Position>(entity);
