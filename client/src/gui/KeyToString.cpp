...
/**
 * @file KeyToString.cpp
 * @brief Utility for converting SFML keyboard keys to string representations
 *
 * SFML (as of version 2.6.1) does not provide a built-in function to convert
 * sf::Keyboard::Key values to human-readable strings (like InputToString).
 * This file implements a manual switch-case mapping for all common keys.
 * If SFML adds such a function in the future, this can be replaced.
 */
...
std::string keyToString(sf::Keyboard::Key key) {
    switch (key) {
        ...
        default: return std::to_string(static_cast<int>(key));
    }
}
