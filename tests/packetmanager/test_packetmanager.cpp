...
public:
    ...
    void assertStringEqual(const std::string &testName, const char *expected, const char *actual,
                           const std::string &description = "") {
...
public:
    ...
    static void transferPacket(PacketManager &sender, PacketManager &receiver) {
        ...
        if (!to_send.empty()) {
            ...
            receiver.handlePacketBytes(raw_packet.data(), raw_packet.size(), (sockaddr_in){});
        }
    }
};
...
void testPacketData(TestRunner &runner) {
    ...
    if (!mailbox.empty()) {
        ...
        runner.assertEqual("Packet data size", (uint32_t) sizeof(super_packet_t), packet->header.data_size,
...
void testMissingPacketsAndAck(TestRunner &runner) {
    ...
    if (packets_to_send.size() > 0) {
        ...
        receiver.handlePacketBytes(raw_packet1.data(), raw_packet1.size(), (sockaddr_in){});
    }
    ...
    if (packets_to_send.size() > 2) {
        ...
        receiver.handlePacketBytes(raw_packet3.data(), raw_packet3.size(), (sockaddr_in){});
    }

    // Check receiver state
    runner.assertEqual("Receiver seqid after gap", 3U, receiver._get_recv_seqid(),
    ...
    if (!ack_packets.empty()) {
        ...
        for (const auto &ack_packet: ack_packets) {
            if (ack_packet->header.ack == 2) {
                ...
                runner.assertEqual("ACK packet type", (uint8_t) 0, ack_packet->header.type,
                                   "ACK packet should have type 0");
                runner.assertEqual("ACK packet seqid", 0U, ack_packet->header.seqid, "ACK packet should have seqid 0");
                runner.assertEqual("ACK packet data_size", 0U, ack_packet->header.data_size,
                                   "ACK packet should have no data");
                runner.assertTrue("ACK packet data is null", ack_packet->data == nullptr,
                                  "ACK packet should have null data");
                break;
            }
        }
    ...
    if (received_packets.size() >= 2) {
        ...
        runner.assertEqual("First received packet seqid", 1U, received_packets[0]->header.seqid,
                           "First packet should be seqid=1");
        runner.assertEqual("Second received packet seqid", 3U, received_packets[1]->header.seqid,
                           "Second packet should be seqid=3");
        ...
        runner.assertStringEqual("Packet 1 data", "Packet 1", data1_received->my_name,
                                 "Packet 1 should contain 'Packet 1'");
        runner.assertStringEqual("Packet 3 data", "Packet 3", data3_received->my_name,
                                 "Packet 3 should contain 'Packet 3'");
    }
...
void testAckPacketRetransmission(TestRunner &runner) {
    ...
    if (second_batch.size() > 0) {
        ...
        receiver.handlePacketBytes(raw_packet2.data(), raw_packet2.size(), (sockaddr_in){});
    }

    // Get ACK packets from receiver
    std::vector<std::unique_ptr<packet_t> > ack_packets = receiver.fetchPacketsToSend();
    runner.assertTrue("Receiver generated ACK packets", !ack_packets.empty(),
                      "Should generate ACK for missing packet 1");

    // Send ACK back to sender to test retransmission logic
    if (!ack_packets.empty()) {
        for (const auto &ack_packet: ack_packets) {
            if (ack_packet->header.ack == 1) {
                // ACK for missing packet 1
                std::vector<uint8_t> ack_raw = PacketManager::serializePacket(*ack_packet);
                sender.handlePacketBytes(ack_raw.data(), ack_raw.size(), (sockaddr_in){});
    ...
    runner.assertTrue("Retransmission packets queued", !retrans_packets.empty(),
                      "Sender should queue packets for retransmission after ACK");
...
void packetWithInvalidSizeIsRejected(TestRunner &runner) {
    ...
    if (!packets_to_send.empty()) {
        ...
        runner.runTest("Invalid packet size rejection", [&]() {
            receiver.handlePacketBytes(raw_packet.data(), raw_packet.size() - 5, (sockaddr_in){});
        });
    }
...
void emptyPacketDataIsHandledCorrectly(TestRunner &runner) {
    ...
    if (!packets.empty()) {
        runner.assertEqual("Empty packet data size", 0U, packets[0]->header.data_size,
                           "Empty packet should have data_size=0");
...
void veryLargePacketIsHandled(TestRunner &runner) {
    ...
    std::vector<std::unique_ptr<packet_t> > packets = manager.fetchPacketsToSend();
    if (!packets.empty()) {
        runner.assertEqual("Large packet data size", (uint32_t) large_size, packets[0]->header.data_size,
                           "Large packet size should be preserved");
    }
...
void sequenceIdOverflowIsHandled(TestRunner &runner) {
    ...
    runner.assertTrue("Sequence ID near overflow handled", manager._get_send_seqid() > 0,
                      "Sequence ID should handle large values");
}
...
void multipleConsecutiveMissingPacketsAreDetected(TestRunner &runner) {
    ...
    for (int i = 1; i <= 5; i++) {
        super_packet_t packet = PacketTestHelper::createTestPacket(20 + i, true,
    ...
    if (packets_to_send.size() >= 5) {
        ...
        receiver.handlePacketBytes(packet1.data(), packet1.size(), (sockaddr_in){});
...
void duplicatePacketsAreHandledCorrectly(TestRunner &runner) {
    ...
    std::vector<std::unique_ptr<packet_t> > received_packets = receiver.fetchReceivedPackets();
    runner.assertEqual("Duplicate packet handling", 2UL, received_packets.size(),
                       "Duplicate packets should be processed");
...
void outOfOrderPacketDeliveryWorks(TestRunner &runner) {
    ...
    if (packets_to_send.size() >= 4) {
        ...
        receiver.handlePacketBytes(packet4.data(), packet4.size(), (sockaddr_in){});
        receiver.handlePacketBytes(packet2.data(), packet2.size(), (sockaddr_in){});
        receiver.handlePacketBytes(packet1.data(), packet1.size(), (sockaddr_in){});
        receiver.handlePacketBytes(packet3.data(), packet3.size(), (sockaddr_in){});
    }
    ...
    if (received_packets.size() >= 4) {
        runner.assertEqual("Packets sorted by seqid", 1U, received_packets[0]->header.seqid,
                           "First packet should have seqid=1");
        runner.assertEqual("Packets sorted by seqid", 2U, received_packets[1]->header.seqid,
                           "Second packet should have seqid=2");
        runner.assertEqual("Packets sorted by seqid", 3U, received_packets[2]->header.seqid,
                           "Third packet should have seqid=3");
        runner.assertEqual("Packets sorted by seqid", 4U, received_packets[3]->header.seqid,
                           "Fourth packet should have seqid=4");
    }
}
